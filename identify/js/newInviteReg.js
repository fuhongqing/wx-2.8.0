$(function () {
    ...
    var thisName=decodeURIComponent(searchArr[0].split('=')[1]);
    var thisMemberId=searchArr[1].split('=')[1];
            ...
            success:function (data) {
                if(data.code=='200'){
                    $.ajax({//提交
                        ...
                        data:JSON.stringify({
                            inviterId:thisMemberId,//	是	string	邀请人id (经纪人id,案场id,市场id)
                            inviteeTelephone:thisPhone,//	是	string	电话
                            type:1//	是	string	1:经纪人提交 2:案场提交 3:市场提交
                        }),
                        success:function (data) {
                            if(data.code=='200'){
                                showTips('提交成功');
                                setTimeout(function () {
                                    $(location).attr('href','download.jsp');
                                },1000);
                            }else{
                                $('.tipModal').show(500);
                            }
