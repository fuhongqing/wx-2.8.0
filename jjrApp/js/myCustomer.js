$(function () {
    ...
    function customerLists() {
        $.ajax({
            url:initUrl+'api/v2/mine/customers',
            type:'get',
            async:false,
            data:{
                companyId:thisparentID,//		否	Long	公司id
                branchId:thisbranchID,//		否	Long	分行id
            ...
            success:function (data) {
                if(data.code==200) {
                    ...
                    $.each(customerData,function (i) {
                        ...
                        if(customerData[i].propertyName){
                            spanHtml='<span>'+customerData[i].propertyName+'</span>';
                        }
                        customerHtml+=`
                        <div class="cus-item" id="${customerData[i].customerId}">
                            <div class="cus-lf">
                                <div class="name"><b>${customerData[i].customerName}</b>${spanHtml}</div>
