	...
	//$("#search").on("keyup",function(){
		...
		$.ajax({
			...
			data: {
				...
				customerState: thiscustomerStatus[3].code,
				...
				if(data.code == 200) {
					if(data.data.totalCount != 0) {
						...
						$.each(data.data.list, function(i) {
							searchStr += "<a href='commission-detail.jsp?" + data.data.list[i].customerState + "&" + data.data.list[i].customerId + "&" + data.data.list[i].transactionNumber +
			...
			success: function(data) {
				...
				if(data.code == 200) {
					if(data.data.totalCount != 0) {
						...
						$(".report-num").html("<span>" + data.data.totalCount + "</span>人");
						$.each(data.data.list, function(i) {
							reportStr += "<a href='commission-detail.jsp?" + data.data.list[i].customerState + "&" + data.data.list[i].customerId + "&" + data.data.list[i].transactionNumber +
								"'><li><div>" + data.data.list[i].customerName + "</div><div>" +
								data.data.list[i].propertyName + "</div><div><span>" +
								data.data.list[i].theDate + "</span></div></li></a>";
						});
